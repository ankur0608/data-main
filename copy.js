...
async function scrapeWithRetry(browser, task) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      console.log(`[${lang}] Scraping ${sign} (Attempt ${attempt}/${MAX_RETRIES}) → ${url}`);
      ...
      console.error(`❌ [${lang}] Failed attempt ${attempt} for ${sign}: ${err.name}`);
      if (page) await page.close(); // Ensure page is closed on failure

      if (attempt === MAX_RETRIES) {
        console.error(`❌ [${lang}] All retries failed for ${sign}. Giving up.`);
        ...
        return {
          ...
          status: "rejected",
          reason: err.message,
          value: { horoscope: null, luckyNumber: null, luckyColor: null, remedy: null, ratings: {} },
        };
      }
...
(async () => {
  ...
  results.forEach(res => {
    if (res) { // Ensure result is not undefined
